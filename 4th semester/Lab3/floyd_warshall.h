...
int floyd_warshall(int start, int end) {
    ...
    int size = _data.cities_count;
    int k;
    for (int i = 0; i < size; i++) { _data.cost_matrix[i][i] = 0; }
    ...
    for (k = 0; k < size; k++)
    {
        for (int i = 0; i < size; i++)
        {
            for (int j = 0; j < size; j++)
            {
                if (_data.cost_matrix[i][k] && _data.cost_matrix[k][j] && i != j)
                {
                    if (_data.cost_matrix[i][k] + _data.cost_matrix[k][j] < _data.cost_matrix[i][j] || _data.cost_matrix[i][j] == 0)
                    {
                        _data.cost_matrix[i][j] = _data.cost_matrix[i][k] + _data.cost_matrix[k][j];
                        distances[j] = _data.cost_matrix[i][k] + _data.cost_matrix[k][j];
                    }                      
                }                   
            }              
        }
